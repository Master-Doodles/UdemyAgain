    ...
    public class Handler(AppDbContext context) : IRequestHandler<Query, Result<List<Photo>>>
    {
        public async Task<Result<List<Photo>>> Handle(Query request, CancellationToken cancellationToken)
        {
            var photos = await context.Users 
                .Where(x => x.Id == request.UserId) //making sure its the photos for the user
                .SelectMany(x => x.Photos)//selects the photos
                .ToListAsync(cancellationToken);//listify's them

            return Result<List<Photo>>.Success(photos); //returns the photos
        }
    }
}